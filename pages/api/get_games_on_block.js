...
export default async (req, res) => {
    try {
        ...
        query = 'SELECT t0.order_id, t1.id, t2.name AS left_name, t2.name_kana AS left_name_kana, t3.name AS right_name, t3.name_kana AS right_name_kana, t4.name AS left_group_name, t5.name AS right_group_name FROM ' + block_name + ' AS t0 LEFT JOIN hokei_man AS t1 ON t0.hokei_man_id = t1.id LEFT JOIN players AS t2 ON t1.left_player_id = t2.hokei_man_player_id LEFT JOIN players AS t3 ON t1.right_player_id = t3.hokei_man_player_id LEFT JOIN groups AS t4 ON t2.group_id = t4.id LEFT JOIN groups AS t5 ON t3.group_id = t5.id';
        const result_block = await conn.query(query);
