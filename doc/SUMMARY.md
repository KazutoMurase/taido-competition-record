# SUMMARY

[はじめに](README.md)

# 開発者向け

- [開発環境構築](develop/develop.md)
- [開発用ツール](develop/tools.md)
- [現在競技の自動進行](develop/playwright.md)
- [大会準備](develop/preparation.md)
- [GCP環境へのデプロイ方法(1からセットアップする場合)](develop/deploy.md)
- [GCP環境へのデプロイ方法(既存のサイトを使いまわす場合)](develop/deploy_using_existing_website.md)
- [GCP環境へのデプロイ方法(既存のプロジェクトで別のサイトを立てる場合)](develop/deploy_another_website.md)
- [ユーティリティ](develop/utility.md)
- [サイトの構成](site.md)
- [配信用の現在試合表示](overlay.md)
- [賞状印刷](admin/certificate.md)
- [結果出力](admin/export.md)

# 役員マニュアル

- [準備](admin/setup.md)
- [選手点呼係](admin/check_players.md)
- [選手呼び出し係](admin/call_players.md)
- [記録係](admin/record.md)
- [司会](admin/announce.md)
