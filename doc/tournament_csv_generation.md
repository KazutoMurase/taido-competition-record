...
配置処理は競技単位で並列実行します。ワーカー数はデフォルトの `--jobs 0` で
CPU数に合わせて自動決定され、`--jobs 1` を指定すると直列実行になります。
Random配置も各競技で同じ `--seed` から独立した乱数列を作るため、並列実行しても
イベント順に依存せず同じ配置を再現できます。

昨年度順位だけをシードとして使い、同じ団体の選手をトーナメント木の
2・4・8・16…分割へ階層的に分散する場合は `--placement balanced` を指定します。
同団体の初戦対戦も避け、厳密な配置が成立しない場合は均等配置、初戦回避の順に
制約を緩和します。
...
`rank_group` に `A`〜`Z` のアルファベットが含まれる場合は、その文字ごとのチームに分けます。
小文字と全角アルファベットも大文字の半角へ正規化し、アルファベットがない場合は地区名だけのチームにします。
