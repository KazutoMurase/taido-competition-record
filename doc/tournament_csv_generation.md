...
smart/balanced 配置では、競技ごとの通算試行数、シード調整中／探索中の区別、
探索ノード数、経過時間、緩和した条件、成功・失敗理由をstderrへ即時表示します。
探索中は約1秒間隔で更新します。並列時も親プロセスがログを集約し、
先に投入した競技の終了を待たずに他競技の進捗を表示します。
追加のPython依存は不要です。

既定値は100試行、1試行20,000探索ノードです。制約の緩和段階に試行回数を
均等配分する既存仕様のため、指定値によっては実試行上限が一致しません。
表示の分母には実際の試行上限を使用します。ノード上限は実行時間の上限ではなく、
進捗表示の追加だけで長時間探索が解消するわけではありません。

### smart配置の順位とブロック

全体順位 `*_rank_total`、推薦順位 `*_rank_lastyear` の順に数値昇順でシードを
選び、最大8人の枠内で位置を固定します。同順位のシード候補同士は、
同団体選手の配置余地を評価して順序を調整します。
残りの枠で団体内順位 `*_rank_group` によるシード位置を優先しますが、
団体内のブロック配置に反する位置には固定しません。

推薦選手のいない団体では、団体内1〜4位を左上・右下・右上・左下に分けます。
団体ごとの左右・上下反転を許容し、1位・2位、3位・4位はそれぞれ対角を保ちます。
欠番は詰めず、記載された順位に対応するブロックを使用します。

推薦選手が1人いる場合は、その位置を基準に配置します。推薦選手が左上なら、
団体内1位は右下、2位は右上、3位は左下、4位は左上です。
推薦選手自身の団体内順位はこの割り当てに重ねて適用しません。
推薦選手が複数いる場合は固定位置を保ち、最優先の推薦選手を基準に、
推薦選手がいないブロックから団体内順位順に埋めます。その後は推薦選手の
いるブロックを使います。同じシード優先順位なら選手IDの文字列順で基準を決めます。
推薦順位を持っていても最大8人から外れた選手は固定シードにはならず、
残りの配置で団体内順位の選手より先に扱います。

この順位別ブロック配置は試行を重ねても緩和しません。全体順位・推薦順位の
固定位置と両立しない場合は、シードを動かさずエラーになります。
5位以下・順位なしの選手は同団体の分散と初戦回避を優先して配置します。
`--placement random` は従来どおり全員をシャッフルしますが、smart/balanced の
失敗時にランダムへ切り替える機能はありません。
...
空白・セル内改行を含む値は二重引用符で囲み、セル内の二重引用符は二重化してください。
標準のCSVリーダーで読み込み、引用符のないセル内改行を推測して補修する処理は行いません。
