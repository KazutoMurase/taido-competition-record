...
function UpdateTableResult({ event_name, id, return_url }) {
  ...
  const onSubmit = (data, values, initialValues, event_name) => {
    ...
    if (values[0] && values[1]) {
      post["main_score"] =
        (parseInt(values[0]) * 10 + parseInt(values[1])) / 10;
    }
    if (values[2] && values[3]) {
      post["sub1_score"] =
        (parseInt(values[2]) * 10 + parseInt(values[3])) / 10;
    }
    if (values[4] && values[5]) {
      post["sub2_score"] =
        (parseInt(values[4]) * 10 + parseInt(values[5])) / 10;
    }
