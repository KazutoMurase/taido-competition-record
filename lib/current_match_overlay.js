...
import { GetCurrentBlockData } from "../pages/api/current_block";
...
function BuildDisplayText(
  ...
  if (participantNames.length === 2 && leftColor) {
    if (leftColor === "red") {
      parts.push(`(赤)${participantNames[0]} vs (白)${participantNames[1]}`);
    } else {
      parts.push(`(赤)${participantNames[1]} vs (白)${participantNames[0]}`);
    }
...
async function LoadCurrentMatchForCourt(court) {
  ...
  let leftColor = null;
  try {
    const blockDataArray = await GetCurrentBlockData({
      block_number: court,
      event_name: current.competition_code,
    });
    const blockData = Array.isArray(blockDataArray)
      ? blockDataArray[0]
      : blockDataArray;
    if (blockData && blockData.left_color) {
      leftColor = blockData.left_color;
    }
  } catch (err) {
    console.error("Failed to fetch left_color from GetCurrentBlockData:", err);
  }
